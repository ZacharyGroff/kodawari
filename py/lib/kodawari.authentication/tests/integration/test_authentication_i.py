...
@pytest.mark.asyncio
async def test_200_request_is_authorized(
    fastapi_test_client: TestClient, secret: str, claims: Dict[str, Any]
) -> None:
...
@pytest.mark.asyncio
async def test_401_expiry_is_in_past(
    fastapi_test_client: TestClient, secret: str, claims: Dict[str, Any]
) -> None:
    claims["expiry"] = 0
    token: str = encode(claims, key=secret, algorithm=_bearer_token_algorithms[0])
...
@pytest.mark.asyncio
async def test_401_token_cannot_be_decoded(
    fastapi_test_client: TestClient, secret: str, claims: Dict[str, Any]
) -> None:
...
@pytest.mark.asyncio
async def test_500_secret_environment_variable_not_set(
    fastapi_test_client: TestClient, claims: Dict[str, Any]
) -> None:
